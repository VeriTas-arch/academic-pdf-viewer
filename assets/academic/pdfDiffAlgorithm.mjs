...
export function mergeTextAndRasterResults(textResult, rasterResult) {
    ...
    const visualChanges = rasterResult.changes.filter(change => [
        ...
        ...change,
        originalRegions: alignVisualRegionsToTextLines(change.originalRegions, textResult.originalRegions),
        modifiedRegions: alignVisualRegionsToTextLines(change.modifiedRegions, textResult.modifiedRegions)
    }));
...
function isRegionCovered(region, coverage) {
...
function alignVisualRegionsToTextLines(regions, textRegions) {
    return regions.map(region => {
        const sameLine = textRegions.filter(textRegion => normalizedRegionsShareTextLine(region, textRegion));
...
function normalizedRegionsShareTextLine(first, second) {
