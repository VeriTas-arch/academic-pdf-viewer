...
(function () {
  ...
  function getDestinationPosition(destArray) {
    const kind = destArray[1] && destArray[1].name;
    if (kind === "XYZ") {
      return { x: destArray[2], y: destArray[3] };
    }
    if (kind === "FitH" || kind === "FitBH") {
      return { x: 0, y: destArray[2] };
    }
    if (kind === "FitV" || kind === "FitBV") {
      return { x: destArray[2], y: null };
    }
    if (kind === "FitR") {
      return { x: destArray[2], y: destArray[5] };
    }
    return { x: 0, y: null };
  }
